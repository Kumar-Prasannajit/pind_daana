...
export default function EditPuriPujaPage() {
    ...
    useEffect(() => {
        const fetchPuriPuja = async () => {
            try {
                const response = await fetch(`/api/puri-pujas/${resolvedParams.id}`);
                if (!response.ok) throw new Error("Failed to fetch puri puja details");
                ...
                // Backwards compatibility for single price or handle pricing array
                if (Array.isArray(data.pricing) && data.pricing.length > 0) {
                    setPricing(data.pricing.map((p: any) => ({ name: p.name, amount: String(p.amount) })));
                } else if (data.price !== undefined) {
                    setPricing([{ name: "Standard", amount: String(data.price) }]);
                }
                if (data.imageUrl) {
                ...
                toast.error("Could not load puri puja details");
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(data.error || "Failed to update puri puja");
            }

            toast.success("Puri Puja updated successfully!");
            router.push("/admin/dashboard/puri-pujas");
            router.refresh();
        } catch (error: any) {
    ...
    const handleDelete = async () => {
        if (!window.confirm("Are you sure you want to delete this puri puja? This action cannot be undone.")) {
            return;
        }
        ...
        try {
            ...
            if (!response.ok) {
                const data = await response.json();
                throw new Error(data.error || "Failed to delete puri puja");
            }

            toast.success("Puri Puja deleted successfully!");
            router.push("/admin/dashboard/puri-pujas");
            router.refresh();
        } catch (error: any) {
            toast.error(error.message);
        ...
        <div className="max-w-4xl mx-auto p-6">
            <div className="flex justify-between items-start mb-8">
                <div>
                    ...
                    <h1 className="text-2xl font-heading font-bold text-gray-900">Edit Puri Puja</h1>
                    <p className="text-gray-500 mt-1 text-sm">Update puri puja details</p>
                </div>
                                ...
                                {imagePreview ? (
                                    ...
                                    <div className="space-y-1 text-center">
                                        ...
                                        <div className="flex text-sm text-gray-600 justify-center">
                                            <label
                                                htmlFor="puri-image-upload-edit"
                                            ...
                                            >
                                                ...
                                                <input
                                                    id="puri-image-upload-edit"
                                                    name="puri-image-upload-edit"
                                                    type="file"
                                                    accept="image/*"
                                                    className="sr-only"
