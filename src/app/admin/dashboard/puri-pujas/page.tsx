...
export default function PuriPujasPage() {
    const [puriPujas, setPuriPujas] = useState<IPuriPuja[]>([]);
    ...
    const handleDelete = async (id: string) => {
        if (!window.confirm("Are you sure you want to delete this puri puja?")) return;

        try {
            const response = await fetch(`/api/puri-pujas/${id}`, {
                method: "DELETE",
            });

            if (!response.ok) {
                throw new Error("Failed to delete puri puja");
            }
            ...
            toast.error("Failed to delete puri puja");
        }
    };

    const filteredPuriPujas = puriPujas.filter(puja =>
    ...
    return (
        <div className="max-w-6xl mx-auto p-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                <div>
                    <h1 className="text-2xl font-heading font-bold text-gray-900">Puri Pujas</h1>
                    <p className="text-gray-500 mt-1 text-sm">Manage pujas specific to Puri</p>
                </div>
                <Link
                    href="/admin/dashboard/add-puri-puja"
            ...
            {puriPujas.length === 0 ? (
                <div className="text-center py-20 bg-white rounded-2xl border border-gray-100 shadow-sm">
                    ...
                    <h3 className="text-lg font-bold text-gray-900 mb-2">No Puri Pujas Found</h3>
                    <p className="text-gray-500 mb-6 max-w-sm mx-auto">Get started by adding your first Puri puja to the platform.</p>
                    <Link
                        href="/admin/dashboard/add-puri-puja"
